...
text = text.replace(stats_old.replace('db.medicines', 'conn.medicines'), stats_new)

# 5. Fix api_medicines GET
get_meds_old = """    total = conn.medicines.count_documents(query)
    ...
    for r in cursor:
        ...
        del r["_id"]
        rows.append(r)"""
get_meds_new = """    total = conn.execute(f"SELECT COUNT(*) FROM medicines {where}", params).fetchone()[0]
    rows = conn.execute(
...
# Final cleanup for remaining str(_id) lines that might have been missed
text = text.replace('r["id"] = str(r["_id"])', '')
text = text.replace('del r["_id"]', '')
